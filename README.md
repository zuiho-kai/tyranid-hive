...
### Demo 1：Solo Mode — 小老板的早晨

```
用户: "帮我看一下今天新能源汽车出口相关新闻，顺便看看宁德时代今早表现，给个调仓判断"

[主脑判断] 顺序工具链 → Solo Mode
           ↓
> 🔍 搜索：新能源汽车出口 最新新闻...
> 📈 获取：CATL 今日行情...
> 📰 检索：欧盟关税 锂电 政策动态...
...
[主脑] 今早有三件事值得注意：
  ...
  一、欧委会宣布补贴调查延期 3 个月，短期利空但市场已部分消化
  二、宁德时代低开 1.2%，成交量偏低，无恐慌性抛售信号
  调仓建议：无明确趋势信号，今天不是高优先级操作时机

  需要设监控，涨跌超 2% 提醒你吗？
```

**系统内部**：主脑串行调用搜索/行情/分析工具，全程无 Submind，上下文一条链跑完。
...
- **核心职责**：赛后复盘 → 失败基因落 Lessons → 优秀基因落 Playbook → 维护基因谱系 → **专化生物形态结晶**
...
这解释了公司制架构（如固定部门制）与虫巢的关系：公司制 = 人工预配成熟期专化形态，跳过了探索和积累阶段。虫巢的优势在于专化形态来自真实数据蒸馏，任务域变化时能自动退化重塑。
...
plugins:
  hive:
    enabled: true
    mode: "auto"          # auto | solo | hive（auto = 主脑自动判断）
...
简单问题 → Solo Mode 直接答；复杂问题 → 主脑自动判断是否触发 Trial / Chain / Swarm。
...
- [x] 双模架构（Solo Mode / Hive Mode 自动切换）
- [x] 四种执行模式（Solo / Trial / Chain / Swarm）
- [x] 四层神经等级（Overmind / Submind / Brood / Unit）
- [x] 进化大师（TrialClosed 事件订阅 + 受限独立意志）
- [x] 条件赛马（赛前论证 + 硬门槛 + 软评分双层收敛）
- [x] 三层基因进化（Constitution / Playbook / Lessons）
- [x] 适存驱动（生物质消耗 / 收割 / 净值 P&L）
- [x] 基因本源（身份连续性 + 谱系追溯）
- [x] 小主脑三态管理（常驻 / 试验 / 休眠）
...
- [ ] 生物质净值曲线可视化
- [ ] Chain Mode UI（阶段进度 + 改动摘要追踪）
- [ ] 进化大师面板（基因谱系 + Playbook 版本历史）
- [ ] PostgreSQL 迁移
