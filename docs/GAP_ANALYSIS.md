...
> 并给出可直接编码的设计规格。
>
> **最后更新：2026-04-02，Phase 1/2/3 已合并主干。**
> 当前缺口见 `docs/NEXT_STEPS.md`。
...
|------|------------|---------|--------|--------|
| 执行模式自动路由 | 主脑判断 Solo/Trial/Chain/Swarm | 主脑路由 + 历史权重覆盖 + Skill 优先匹配 | **80%** | ~~P0~~ |
| 收敛引擎（Trial 评分） | 硬门槛 + 六维软评分 | 硬门槛 + 启发式评分 | **70%** | ~~P1~~ |
| Submind 实体 | L2 域内 CEO，三态管理，跨域协商 | 模型已建，Credit 分账已接入 | **50%** | P2 |
| 生物质消耗侧 | Drain = 待机+执行+协调 token 成本 | step 粒度 Drain + 分账 | **75%** | ~~P3~~ |
| 进化大师增强 | 两阶段复盘 + TrialClosed 订阅 + Skill 自合成 | Reflect+Distill+器官结晶+语义审计 | **80%** | ~~P4~~ |
| 赛前论证 | ~500 token 快速推理决定是否开赛 | 不存在 | 0% | P0 子项（待补） |
| 失败分类 | 环境/理解/策略/质量 四级惩罚 | error_class 字段已有，惩罚逻辑未完全接入 | 40% | P3 子项 |
| 基因本源追踪 | GeneSeed 谱系连续性决定身份 | 模型字段存在，实际追踪未实现 | 20% | P2 子项 |
| Playbook 语义聚类审计 | 冗余检测 + 孤岛标记 + 域边界 | SemanticAuditor 已实现（关键词版） | **70%** | ~~P4 子项~~ |
| Policy 进化系统 | — | PolicyRegistry + Shadow + Decay 完整闭环 | **85%** | 新增 |
| Episode 留痕系统 | — | EpisodeStore + TaskFingerprint + step 记账 | **90%** | 新增 |
| 器官结晶 | 专化生物形态蒸馏 | OrganCrystallizer + SkillRegistry | **75%** | 新增 |
| 世界模型 | 格式塔共享结构化状态 | WorldModelService 极简版（未接入执行流） | **30%** | 新增 |
| Discord 式三栏 UI | Trunk/Trial/Hive/Ledger 四频道 | React 脚手架 | 5% | P5（本轮不做） |

**整体从 README 愿景角度：从 ~15% → ~70%**
